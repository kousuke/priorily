...
contract Priorily {
  ...
  modifier noReentrant() {
    require(!noReentrantLock, "No re-entrancy");
    noReentrantLock = true;
    _;
    noReentrantLock = false;
  }

    function setTreasury(address addr) external noReentrant {
    ...
    function savePlan(uint planId, string memory title, uint planType) payable external requireOwner noReentrant returns(Plan memory){
      plans[planId].title = title;
      plans[planId].planType = planType;
      return plans[planId];
    }

    function createPlan(string memory title, uint planType) payable external requireOwner noReentrant returns(uint){
    ...
    function suggestPlan(string memory title, uint planType) payable external noReentrant returns(uint){
    ...
    function vote(uint planId) payable external noReentrant {
